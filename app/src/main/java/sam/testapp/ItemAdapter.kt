...
class ItemAdapter(val viewActions: onViewSelectedListener, shopItems: MutableList<Item>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    inner class ItemViewHolder(parent: ViewGroup) : RecyclerView.ViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.shop_item,parent,false)) {
        fun bind(item: Item) = with(itemView) {
            ...
            textView3.text = "Stat Requirments: Str ${item.stat_requirement.strength_lower}-${item.stat_requirement.strength_upper}  Dex ${item.stat_requirement.dexterity_lower}-${item.stat_requirement.dexterity_upper}  Int ${item.stat_requirement.intelligence_lower}-${item.stat_requirement.intelligence_upper}"
            textView4.text = "${item.ability.type}, Range ${item.ability.findRange()}, Flexibility ${item.ability.relative_pairs.size}"
            imageView_item_image.setImageResource(R.drawable.item_image_helmet)
